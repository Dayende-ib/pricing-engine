<div class="space-y-6">
    <div class="flex items-center justify-between gap-4">
        <div>
            ...
            <flux:text size="sm" class="mt-0.5 text-zinc-500 dark:text-zinc-400">Urgence, complexité technique, négociation...</flux:text>
        </div>
    ...
    @if($project->pricingAdjustments->isEmpty())
        <div class="flex flex-col items-center px-6 py-16 text-center">
            <div class="flex size-16 items-center justify-center rounded-2xl bg-zinc-100 dark:bg-white/[0.04]">
                <flux:icon icon="adjustments-horizontal" variant="outline" class="size-8 text-zinc-300 dark:text-zinc-600" />
            </div>
            <flux:heading class="mt-4">Aucun ajustement</flux:heading>
            <flux:text class="mt-1 max-w-md text-zinc-500 dark:text-zinc-400">Ajoutez des ajustements pour l'urgence, la complexité, la négociation...</flux:text>
    ...
    @else
                        ...
                        <flux:table.row>
                            <flux:table.cell variant="strong">
                                <div class="flex items-center gap-3">
                                    <div class="flex size-9 shrink-0 items-center justify-center rounded-lg bg-violet-50 dark:bg-violet-500/10">
                                        <flux:icon
                                    ...
                                    <div class="min-w-0">
                                        <div class="truncate font-medium text-zinc-900 dark:text-[#f0f0f2]">{{ $adj->name }}</div>
                                        @if($adj->reason)
                                            <div class="truncate text-sm text-zinc-500 dark:text-zinc-400">{{ $adj->reason }}</div>
                                        @endif
                                    </div>
                                </div>
                            </flux:table.cell>
                            ...
                            <flux:table.cell variant="strong" class="hidden sm:table-cell">
                                <span class="font-semibold text-zinc-900 dark:text-[#f0f0f2]">{{ number_format($impact, 0, ',', ' ') }} {{ $project->currency }}</span>
                            </flux:table.cell>
        ...
        <div class="flex justify-end">
            <div class="rounded-xl bg-zinc-100 px-5 py-4 text-right dark:bg-white/[0.04]">
                <div class="text-xs font-semibold uppercase tracking-widest text-zinc-400">Total ajustements</div>
                <div class="mt-1 text-2xl font-bold text-zinc-900 dark:text-[#f0f0f2]">
